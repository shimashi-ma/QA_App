...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    //コンパニオンオブジェクト　companion object {...}　　Java言語におけるstaticメンバ？
    companion object {
        private val PERMISSIONS_REQUEST_CODE = 100  //パーミッション用
        private val CHOOSER_REQUEST_CODE = 100  //パーミッション用
    }

    private var mGenre: Int = 0  //質問のジャンル用
    private var mPictureUri: Uri? = null  //画像保存用
    ...
    override fun onClick(v: View) {
        if (v === imageView) {
            // パーミッションの許可状態を確認する Android6.0以降か否か
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    // 許可されている
                    //Intent連携でギャラリーとカメラを選択するダイアログを表示させるshowChooserメソッド
                    showChooser()
                } else {
            ...
            // 添付画像を取得する
            //as? 安全なキャスト演算子 キャストに失敗したら null を返します。画像が設定されていない場合にキャストしようとするとアプリが落ちるため、as? を使って、画像が無いときは null を返すようにしています。
